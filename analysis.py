...
def read_ejected_expelled(sim, haloid):
    key = f'{sim}_{str(int(haloid))}'
    ejected = pd.read_hdf('/home/lonzaric/astro_research/ahdata/ejected_particles.hdf5', key=key)
    cooled = pd.read_hdf('/home/lonzaric/astro_research/ahdata/cooled_particles.hdf5', key=key)
    expelled = pd.read_hdf('/home/lonzaric/astro_research/ahdata/expelled_particles.hdf5', key=key)
    accreted = pd.read_hdf('/home/lonzaric/astro_research/ahdata/accreted_particles.hdf5', key=key)
...
def read_all_ejected_expelled():
    ...
    for key in keys:
        ...
        ejected1 = pd.read_hdf('/home/lonzaric/astro_research/ahdata/ejected_particles.hdf5', key=key)
        ejected1['key'] = key
        ejected = pd.concat([ejected, ejected1])
        cooled1 = pd.read_hdf('/home/lonzaric/astro_research/ahdata/cooled_particles.hdf5', key=key)
        cooled1['key'] = key
        cooled = pd.concat([cooled, cooled1])
        expelled1 = pd.read_hdf('/home/lonzaric/astro_research/ahdata/expelled_particles.hdf5', key=key)
        expelled1['key'] = key
        expelled = pd.concat([expelled, expelled1])
        accreted1 = pd.read_hdf('/home/lonzaric/astro_research/ahdata/accreted_particles.hdf5', key=key)
        accreted1['key'] = key
...
def read_ram_pressure(sim, haloid):
    ...
    key = f'{sim}_{haloid}'
    data = pd.read_hdf('/home/lonzaric/astro_research/ahdata/ram_pressure.hdf5', key=key)
