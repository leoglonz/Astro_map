...
def get_keys():
    path1 = '~/Desktop/Stellar_Feedback_Code/SNeData/ejected_particles.hdf5'
...
#     path1 = '~/Desktop/Stellar_Feedback_Code/SNeData/tracked_particles.hdf5'
...
def read_tracked_particles(sim, haloid, verbose=False):
    ...
    # import the tracked particles dataset
    path1 = '~/Desktop/Stellar_Feedback_Code/SNeData/tracked_particles_v2.hdf5'
    data = pd.read_hdf(path1, key=key)
...
#     expelled = expelled.apply(calc_angles_tidal, axis=1)
    ...
    if save:
        key = f'{sim}_{str(int(haloid))}'
        filepath = '~/Desktop/Stellar_Feedback_Code/SNeData/ejected_particles.hdf5'
        print(f'Saving {key} ejected particle dataset to {filepath}')
        ejected.to_hdf(filepath, key=key)
        
        filepath = '~/Desktop/Stellar_Feedback_Code/SNeData/cooled_particles.hdf5'
        print(f'Saving {key} cooled particle dataset to {filepath}')
        cooled.to_hdf(filepath, key=key)

        filepath = '~/Desktop/Stellar_Feedback_Code/SNeData/expelled_particles.hdf5'
        print(f'Saving {key} expelled particle dataset to {filepath}')
        expelled.to_hdf(filepath, key=key)
                
        filepath = '~/Desktop/Stellar_Feedback_Code/SNeData/accreted_particles.hdf5'
...
def read_ejected_expelled(sim, haloid):
    key = f'{sim}_{str(int(haloid))}'
    ejected = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/ejected_particles.hdf5', key=key)
    cooled = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/cooled_particles.hdf5', key=key)
    expelled = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/expelled_particles.hdf5', key=key)
    accreted = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/accreted_particles.hdf5', key=key)
...
def read_all_ejected_expelled():
    ...
    for key in keys:
        ...
        ejected1 = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/ejected_particles.hdf5', key=key)
        ejected1['key'] = key
        ejected = pd.concat([ejected, ejected1])
        cooled1 = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/cooled_particles.hdf5', key=key)
        cooled1['key'] = key
        cooled = pd.concat([cooled, cooled1])
        expelled1 = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/expelled_particles.hdf5', key=key)
        expelled1['key'] = key
        expelled = pd.concat([expelled, expelled1])
        accreted1 = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/accreted_particles.hdf5', key=key)
        accreted1['key'] = key
...
def read_ram_pressure(sim, haloid):
    ...
    path = '~/Desktop/Stellar_Feedback_Code/SNeData/ram_pressure.hdf5'
    key = f'{sim}_{haloid}'
    data = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/ram_pressure.hdf5', key=key)
...
def calc_discharged(sim, haloid, save=True, verbose=True):
    ...
    if save:
        key = f'{sim}_{str(int(haloid))}'
        filepath = '~/Desktop/Stellar_Feedback_Code/SNeData/predischarged_particles.hdf5'
        print(f'Saving {key} predischarged particle dataset to {filepath}')
        predischarged.to_hdf(filepath, key=key)
 
        filepath = '~/Desktop/Stellar_Feedback_Code/SNeData/discharged_particles.hdf5'
        print(f'Saving {key} discharged particle dataset to {filepath}')
        discharged.to_hdf(filepath, key=key)
        
        filepath = '~/Desktop/Stellar_Feedback_Code/SNeData/dsrg_accreted_particles.hdf5'
        print(f'Saving {key} accreted particle dataset to {filepath}')
...
def calc_dsrg_heated(sim, haloid, save=True, verbose=True):
    ...
    if save:
        key = f'{sim}_{str(int(haloid))}'
        filepath = '~/Desktop/Stellar_Feedback_Code/SNeData/preheated_particles.hdf5'
        print(f'Saving {key} preheated particle dataset to {filepath}')
        preheated.to_hdf(filepath, key=key)
        
        filepath = '~/Desktop/Stellar_Feedback_Code/SNeData/heated_particles.hdf5'
...
def read_discharged():
    ...
    for i,key in enumerate(keys):
        ...
        sim = key[:4]
        haloid = int(key[5:])
        predischarged1 = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/predischarged_particles.hdf5',\
             key=key)
        predischarged1['key'] = key
        predischarged = pd.concat([predischarged, predischarged1])
        
        discharged1 = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/discharged_particles.hdf5', key=key)
        discharged1['key'] = key
        discharged = pd.concat([discharged, discharged1])
        
        dsrg_accreted1 = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/dsrg_accreted_particles.hdf5',\
            key=key)
        dsrg_accreted1['key'] = key
        dsrg_accreted = pd.concat([dsrg_accreted, dsrg_accreted1])
  
        preheated1 = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/preheated_particles.hdf5', key=key)
        preheated1['key'] = key
        preheated = pd.concat([preheated, preheated1])
        
        heated1 = pd.read_hdf('~/Desktop/Stellar_Feedback_Code/SNeData/heated_particles.hdf5', key=key)
        heated1['key'] = key
